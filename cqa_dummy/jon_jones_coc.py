...
# Replace with your actual Polygon.io API key and desired CSV filename
API_KEY = 'GNthmWT9qYGm57QwnIJ_orim_uN5mbc0'
CSV_FILENAME = 'umpy_glumpy.csv'
...
# Function to detect market regimes using HMM with error handling
def detect_market_regimes(returns):
    try:
        model = GaussianHMM(n_components=2, covariance_type="diag", n_iter=1000, init_params='', params='stmc')
        # Manually initialize parameters
        model.startprob_ = np.array([0.5, 0.5])
        model.transmat_ = np.array([[0.95, 0.05],
                                    [0.05, 0.95]])
        model.means_ = np.array([[returns.mean()], [returns.mean()]])
        model.covars_ = np.array([[returns.var()], [returns.var()]])
        model.fit(returns.reshape(-1, 1))
        hidden_states = model.predict(returns.reshape(-1, 1))
        return hidden_states
    except Exception as e:
        print(f"Error in detect_market_regimes: {e}")
        # Fallback: Assume a default regime (e.g., Bull Market)
        return np.array([0] * len(returns))
...
def main():
    ...
    required_positions = 82  # 41 long and 41 short
    if len(valid_tickers) < required_positions:
        raise ValueError(f"Not enough stocks with valid data to select {required_positions//2} long and {required_positions//2} short positions.")
    ...
    hidden_states = detect_market_regimes(market_returns)
    regime = hidden_states[-1]  # Current market regime

    print(f"Current market regime detected: {'Bull' if regime == 0 else 'Bear'} Market")

    # Adjust expected returns based on market regime
    if regime == 0:
        # Bull market adjustments
        expected_returns = returns_df.mean() * 1.2
    else:
        # Bear market adjustments
        expected_returns = returns_df.mean() * 0.8
    ...
    for ticker in valid_tickers:
        ...
        latest_z = z.iloc[-1]
        if pd.isna(latest_z):
            print(f"Skipping {ticker} due to NaN Z-score")
            continue
        z_scores[ticker] = latest_z

    # Ensure enough tickers after Z-score calculation
    if len(z_scores) < required_positions:
        print(f"Not enough tickers after Z-score calculation to select {required_positions//2} long and {required_positions//2} short positions.")
        # Optionally, you can adjust the number of positions or handle this case as needed
        # For now, we'll proceed and let the selection fail if insufficient
        if len(z_scores) < required_positions:
            raise ValueError(f"Not enough tickers after Z-score calculation to select {required_positions//2} long and {required_positions//2} short positions.")
    ...
    if missing_tickers:
        ...
        if len(long_tickers) < num_positions or len(short_tickers) < num_positions:
    ...
    max_weight_per_stock = 0.05      # 5% maximum weight per stock
    ...
    constraints = [
        ...
        w >= -max_weight_per_stock,                        # No position less than -5% of total portfolio
        w <= max_weight_per_stock,                         # No position greater than 5% of total portfolio
    ]
    ...
    desired_cash_ratio = 0.045
    if abs(cash_ratio - desired_cash_ratio) > 0.001:
        print(f"Cash ratio is {cash_ratio:.4f}, adjusting positions to achieve 4.5% cash...")
        adjustment_factor = (total_portfolio_value * (1 - desired_cash_ratio)) / invested_capital
    ...
    dollar_neutrality_ratio = total_long_value / total_short_value if total_short_value != 0 else np.nan
    ...
    if not (0.9 <= dollar_neutrality_ratio <= 1.1):
        ...
        if total_long_value > 0 and total_short_value > 0:
            ...
            print("Cannot adjust dollar neutrality due to zero short or long positions.")
    ...
    if num_long_nonzero < num_positions or num_short_nonzero < num_positions:
        ...
        # For positions with zero shares, set shares to 1 (for long) or -1 (for short)
    ...
    portfolio.to_csv(CSV_FILENAME, index=False)
    print(f"Portfolio has been saved to '{CSV_FILENAME}'.")
